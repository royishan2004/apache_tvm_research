...
function profileTableName(profile) {
    // Use a sanitized table key (no spaces or special chars) to form the table name.
    const key = tableKey(profile);
    // Join with an underscore to avoid spaces and keep it short.
    return clampIdentifier(`${key}_${TABLE_SUFFIX}`);
}
function legacyProfileTableName(profile) {
    return clampIdentifier(`${profile} - ${TABLE_SUFFIX}`);
}
