...
export const bertMatmulResults = pgTable(
  "bert_matmul_results",
  {
    // identity
...
export const bestSchedulePredictions = pgTable(
  "best_schedule_predictions",
  {
    id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
    ingestedAt: timestamp("ingested_at", { withTimezone: true }).defaultNow().notNull(),
  ...
  (table) => [
    uniqueIndex("uniq_best_schedule_predictions_shape").on(
      table.kernel,
      table.m,
      table.k,
      table.n,
...
export type BestSchedulePredictionRow = typeof bestSchedulePredictions.$inferSelect;
export type NewBestSchedulePredictionRow = typeof bestSchedulePredictions.$inferInsert;

export const bestScheduleTransformations = pgTable(
  "best_schedule_transformations",
  {
    id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
    ingestedAt: timestamp("ingested_at", { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
    ...
    kernel: text("kernel").notNull(),
    shapeM: integer("M").notNull(),
    shapeK: integer("K").notNull(),
    shapeN: integer("N").notNull(),

    annotateMetaScheduleParallel: text("annotate.meta_schedule.parallel").notNull().default(""),
    annotateMetaScheduleTilingStructure: text("annotate.meta_schedule.tiling_structure").notNull().default(""),
    annotateMetaScheduleUnrollExplicit: text("annotate.meta_schedule.unroll_explicit").notNull().default(""),
    annotateMetaScheduleVectorize: text("annotate.meta_schedule.vectorize").notNull().default(""),
    annotatePragmaAutoUnrollMaxStep: text("annotate.pragma_auto_unroll_max_step").notNull().default(""),
    annotatePragmaUnrollExplicit: text("annotate.pragma_unroll_explicit").notNull().default(""),
    cacheWriteStorageScope: text("cache_write.storage_scope").notNull().default(""),
    decomposeReductionLoop: text("decompose_reduction.loop").notNull().default(""),
    fuseLoops: text("fuse.loops").notNull().default(""),
    parallelLoop: text("parallel.loop").notNull().default(""),
    reverseComputeAtIndex: text("reverse_compute_at.index").notNull().default(""),
    sampleCategoricalCandidates: text("sample_categorical.candidates").notNull().default(""),
    sampleCategoricalDecision: text("sample_categorical.decision").notNull().default(""),
    samplePerfectTileL2: text("sample_perfect_tile.l2").notNull().default(""),
    samplePerfectTileL3: text("sample_perfect_tile.l3").notNull().default(""),
    samplePerfectTileL4: text("sample_perfect_tile.l4").notNull().default(""),
    splitL2Factors: text("split.l2.factors").notNull().default(""),
    splitL3Factors: text("split.l3.factors").notNull().default(""),
    splitL4Factors: text("split.l4.factors").notNull().default(""),
    vectorizeLoop: text("vectorize.loop").notNull().default(""),
  },
  (table) => [
    index("idx_best_schedule_transformations_shape").on(
      table.kernel,
      table.shapeM,
      table.shapeK,
      table.shapeN,
    ),
    index("idx_best_schedule_transformations_profile_kernel").on(table.profile, table.kernel),
  ]
)

export type BestScheduleTransformationRow = typeof bestScheduleTransformations.$inferSelect;
export type NewBestScheduleTransformationRow = typeof bestScheduleTransformations.$inferInsert;

export const bestPrunedConfig = pgTable(
  "best_pruned_config",
  {
    id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
    ingestedAt: timestamp("ingested_at", { withTimezone: true }).defaultNow().notNull(),
...
export type BestPrunedConfigRow = typeof bestPrunedConfig.$inferSelect;
export type NewBestPrunedConfigRow = typeof bestPrunedConfig.$inferInsert;

export const pruningExperiments = pgTable(
  "pruning_experiments",
  {
    id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
    ingestedAt: timestamp("ingested_at", { withTimezone: true }).defaultNow().notNull(),
    ...
    latestPruningRun: jsonb("latest_pruning_run").$type<Record<string, unknown>>().notNull().default(sql`'{}'::jsonb`),
