...
const MAX_TABLE_SUFFIX_LEN = Math.max(
  ...
  BEST_SCHEDULE_TRANSFORMATIONS_TABLE_SUFFIX.length,
...
type BestScheduleTransformationWideColumn =
  typeof BEST_SCHEDULE_TRANSFORMATION_WIDE_COLUMNS[number]
...
type BestScheduleTransformationInsertRow = {
  profile: string
  kernel: string
  m: number
  k: number
  n: number
  values: Record<BestScheduleTransformationWideColumn, string>
}
...
type ComparisonResultInsertRow = {
  ...
  baselineLatencyUs: number | null
  candidateLatencyUs: number | null
  baselineTaskTuneTime: string
  candidateTaskTuneTime: string
  baselineTaskTuneTimeSec: number | null
  candidateTaskTuneTimeSec: number | null
  latencyRetention: number | null
  execTimeReduction: number | null
...
const uploadBestScheduleTransformationsRoute = createRoute({
  ...
  description:
    'Accepts best_schedule_transformations.json and refreshes a profile-scoped best_schedule_transformations snapshot.',
  request: {
    body: {
      content: {
        'multipart/form-data': {
...
uploadRouter.openapi(uploadBestScheduleTransformationsRoute, async (c) => {
  ...
  const rows: BestScheduleTransformationInsertRow[] = []
  const errors: Array<{ index: number; error: string }> = []
  ...
  for (let i = 0; i < parsed.length; i++) {
    ...
    parseBestScheduleTransformationEntry(
      entry as Record<string, unknown>,
      i,
      rows,
      errors,
...
const uploadComparisonResultsRoute = createRoute({
  ...
  description:
    'Accepts comparison_results.json payload and stores profile-scoped comp_summary snapshot rows.',
  request: {
    body: {
      content: {
        'multipart/form-data': {
...
function parseBestScheduleTransformationEntry(
  entry: Record<string, unknown>,
  index: number,
  rows: BestScheduleTransformationInsertRow[],
  errors: Array<{ index: number; error: string }>,
) {
  const kernel = typeof entry.kernel === 'string' ? entry.kernel.trim() : ''
  const m = finiteInteger(entry.M ?? entry.m)
  ...
  const values = {} as Record<BestScheduleTransformationWideColumn, string>
...
function parseComparisonSummaryPayload(
  payload: unknown,
  ...
  if (
    Array.isArray(payload.shape_comparison_table)
...
function appendComparisonSummaryRows(
  ...
  for (let i = 0; i < shapeRowsRaw.length; i++) {
    ...
    const baselineTaskTuneTimeSec = finiteNumber(
      row.baseline_execution_time_sec ?? row.baseline_task_tune_time_sec,
    )
    const candidateTaskTuneTimeSec = finiteNumber(
      row.candidate_execution_time_sec ?? row.candidate_task_tune_time_sec,
    )
  ...
  const baselineTotalSec = finiteNumber(
    summary.baseline_total_tune_tir_time_sec ?? summary.baseline_execution_time_sec,
  )
  const candidateTotalSec = finiteNumber(
    summary.candidate_total_tune_tir_time_sec ?? summary.candidate_execution_time_sec,
  )
  const baselineHuman =
    typeof summary.baseline_total_tune_tir_time_human === 'string'
      ? summary.baseline_total_tune_tir_time_human
      : formatDurationHuman(baselineTotalSec)
  const candidateHuman =
    typeof summary.candidate_total_tune_tir_time_human === 'string'
...
function buildShapeLabel(entry: Record<string, unknown>, index: number): string {
  const shapeRaw = typeof entry.shape === 'string' ? entry.shape.trim() : ''
  if (shapeRaw) return shapeRaw
...
function toWideTableCell(value: unknown): string {
...
async function ensureBestScheduleTransformationsProfileTable(
  profile: string,
): Promise<{ tableName: string; tableKey: string }> {
  ...
  const legacyProfileName = legacyProfileTableNameForSuffix(
    profile,
    BEST_SCHEDULE_TRANSFORMATIONS_TABLE_SUFFIX,
  )
  if (legacyProfileName !== tableName) {
    const legacyProfileExists = await tableExists(legacyProfileName)
    const targetExists = await tableExists(tableName)
  ...
  if (profile === DEFAULT_PROFILE.toLowerCase()) {
    ...
    if (legacyExists && !targetExists) {
      await execute(
        sql`alter table ${sql.identifier(BEST_SCHEDULE_TRANSFORMATIONS_TABLE_SUFFIX)} rename to ${sql.identifier(tableName)}`
      )
    }
  }
  ...
  const wideColumnDefinitions = sql.join(
    BEST_SCHEDULE_TRANSFORMATION_WIDE_COLUMNS.map(
      (column) => sql`${sql.identifier(column)} text not null default ''`,
    ),
    sql`, `,
  )
  ...
  const migrationAssignments = sql.join(
    BEST_SCHEDULE_TRANSFORMATION_WIDE_COLUMNS.map(
      (column) => sql`
...
async function ensureComparisonResultsProfileTable(
  ...
  if (oldProfileTableName !== tableName) {
    ...
    if (oldExists && !targetExists) {
      await execute(
        sql`alter table ${sql.identifier(oldProfileTableName)} rename to ${sql.identifier(tableName)}`
      )
    }
  }

  const oldLegacyProfileTableName = legacyProfileTableNameForSuffix(
    profile,
  ...
  if (oldLegacyProfileTableName !== tableName) {
    ...
    if (oldLegacyExists && !targetExists) {
      await execute(
        sql`alter table ${sql.identifier(oldLegacyProfileTableName)} rename to ${sql.identifier(tableName)}`
      )
    }
  }
  ...
  if (profile === DEFAULT_PROFILE.toLowerCase()) {
    ...
    if (oldDefaultExists && !targetExists) {
      await execute(
        sql`alter table ${sql.identifier(LEGACY_COMPARISON_RESULTS_TABLE_SUFFIX)} rename to ${sql.identifier(tableName)}`
      )
    }
  }
  ...
  const legacyCompareIdIndex = clampIdentifier(
    `uniq_${key}_${LEGACY_COMPARISON_RESULTS_TABLE_SUFFIX}_compare_id`,
  )
...
async function insertBestScheduleTransformationRows(
  tableName: string,
  rows: BestScheduleTransformationInsertRow[],
  dedupe: boolean,
): Promise<{ inserted: number; duplicates: number }> {
