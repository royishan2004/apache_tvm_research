...
function getIdleDisconnectMs(): number {
...
function getActivityCheckMs(): number {
...
function hasActiveTestProcess(processSnapshot: string): boolean {
...
async function detectActiveTestProcess(): Promise<boolean> {
	try {
		const { stdout } = await execFileAsync(
			"pgrep",
			["-af", TEST_PROCESS_GREP_PATTERN],
			{ maxBuffer: 512 * 1024 },
		);
...
async function checkTestActivity(): Promise<void> {
	try {
		const now = Date.now();
...
export function startDbIdleMonitor(): void {
	if (monitorTimer) {
		return;
	}
	...
	lastActiveTestProcessAt = Date.now();
	console.log(
		`[db-monitor] Active. Disconnect timeout=${Math.round(
			getIdleDisconnectMs() / 1000,
		)}s, check interval=${Math.round(getActivityCheckMs() / 1000)}s (${istTimestamp()})`
	);
...
export function stopDbIdleMonitor(): void {
	if (!monitorTimer) {
		return;
	}
